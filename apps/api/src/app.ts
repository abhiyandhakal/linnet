...
import { activityEvents, actions, betaInvitations, calendarConnections, calendarEvents, calendarOauthStates, createDb, jobs, notificationDevices, notifications } from "@linnet/db";
import { auth } from "./auth";
import { encryptSecret, exchangeGoogleCode, googleConnectUrl, scheduleFocusBlock } from "./calendar";
import { applyProposal, captureInbox, createGoal, createPlanRevision, getGoal, listGoals } from "./services";

const config = getConfig();
const db = createDb(config.DATABASE_URL);
const error = (code: string, message: string, details?: unknown) => ({ code, message, details });
...
export const app = new Elysia()
  ...
  .get("/ready", async () => { await db.execute(sql`select 1`); return { status: "ready" }; })
  .get("/v1/me", async ({ request, status }) => {
  ...
  .get("/v1/goals", async ({ getUser, status }) => { const user = await getUser(); return user ? listGoals(db, user.id) : status(401, error("UNAUTHORIZED", "An invited Linnet session is required.")); })
  .post("/v1/goals", async ({ getUser, body, set, status }) => { const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required.")); set.status = 201; return createGoal(db, user.id, { ...body, targetDate: body.targetDate ? new Date(body.targetDate) : undefined }); }, {
    body: t.Object({ title: t.String(), description: t.Optional(t.String()), why: t.Optional(t.String()), targetDate: t.Optional(t.String()) })
  })
  .get("/v1/goals/:id", async ({ getUser, params, status }) => {
  ...
  .post("/v1/goals/:id/plans", async ({ getUser, params, body, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .post("/v1/inbox", async ({ getUser, body, set, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .get("/v1/jobs/:id", async ({ getUser, params, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .get("/v1/actions", async ({ getUser, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
    return db.select().from(actions).where(eq(actions.userId, user.id)).orderBy(actions.scheduledFor);
  })
  .post("/v1/goals/:id/updates", async ({ getUser, params, body, set, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .post("/v1/goals/:id/conversation", async ({ getUser, params, body, set, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
    const goal = await getGoal(db, user.id, params.id); if (!goal) return status(404, error("NOT_FOUND", "Goal not found."));
  ...
  .post("/v1/assistant", async ({ getUser, body, set, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .post("/v1/calendar/google/connect", async ({ getUser, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .get("/v1/calendar/google/callback", async ({ query, status, set }) => {
  ...
  .post("/v1/calendar/focus-blocks", async ({ getUser, body, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .post("/v1/notifications/devices", async ({ getUser, body, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
  ...
  .get("/v1/reviews/daily", async ({ getUser, status }) => {
    const user = await getUser(); if (!user) return status(401, error("UNAUTHORIZED", "An invited Linnet session is required."));
    const goalList = await listGoals(db, user.id);
